<!-- Footer Section Starts -->
<footer class="footer text-center">
    <!-- Container Starts -->
    <div class="container">
        <!-- Copyright Text Starts -->
        <p>SARAS DAVINA - Ivy The Wonders</p>
        <p>
            &copy; Copyright 2020 Saras Davina with <span class="heart">&#10084;</span> by <a target="_blank" href="">Ivy The Wonders</a>
        </p>
        <!-- Copyright Text Ends -->
        <!-- Social Media Links Starts -->
        <div class="social-icons">
            <ul class="social">
                <li>
                    <a class="facebook" href="https://facebook.com/Saras-Davina-Ivy-The-Wonders-110257024151678/" title="facebook"></a>
                </li>
                <li>
                    <a class="instagram" href="https://www.instagram.com/sarasdavina/" title="instagram"></a>
                </li>
                <li>
                    <a class="globe" href="<?= base_url('/');?>" title="globe"></a>
                </li>
            </ul>
        </div>
        <!-- Social Media Links Ends -->
        <p>Dilarang melanggar Hak Cipta, bukan investasi dan bukan MLM, serta hasil berbeda tiap orang.</p>
    </div>
    <!-- Container Ends -->
    </footer>
    <!-- Footer Section Starts -->
    <!-- Back To Top Starts -->
    <div id="back-top-wrapper" class="d-none d-sm-block">
    <p id="back-top">
        <a href="index.html#top"><span></span></a>
    </p>
    </div>
    <!-- Back To Top Ends -->