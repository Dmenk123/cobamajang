...
class T_klaim_verify extends CI_Model
{
	...
	{
		$this->db->select("tka.*, mu.nama_lengkap, mu.email");
		$this->db->from('t_klaim_agen as tka');
		$this->db->join('m_user mu', 'tka.kode_agen = mu.kode_agen', 'left');
		
		if ($arr_data['status'] == 'belum') {
			$this->db->where("tka.id_user_verify is null");
		}else if($arr_data['status'] == 'belum'){
